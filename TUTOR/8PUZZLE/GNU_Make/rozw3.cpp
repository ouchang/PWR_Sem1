...
int Rozwiazanie3(int T[4][4], int x, int y) {
    priority_queue <Wezel*, vector<Wezel*>, cmp> Q;

    struct Wezel* korzen = nowyWezel3(T, x, y, x, y, 0, NULL, true);
    ...
    while(!Q.empty()) {
        ...
        for(int i=0; i<4; i++) { 
            if(CzyRuchMozliwy(akt->x, akt->y, r_x[i], r_y[i])) {
                ...
                struct Wezel* dziecko = nowyWezel3(T, akt->x, akt->y, akt->x + r_x[i], akt->y + r_y[i], akt->g + 1, akt, false);
                Q.push(dziecko); 
            }
        }
    }
